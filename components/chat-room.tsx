...
export function ChatRoom({ secret }: ChatRoomProps) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(connectedRef, (snap) => {
      ...
      console.log("Connection status:", isCurrentlyConnected);
  ...
  useEffect(() => {
    ...
    const typingUnsubscribe = onValue(typingRef, (snapshot) => {
      ...
      if (data) {
        ...
        setTypingUsers(
          typingArray.filter(
            (user) => user.content && user.userId !== auth.currentUser?.uid
          )
        );
      } else {
        setTypingUsers([]);
    ...
    <div>
      ...
      <div className="flex flex-col items-center justify-center min-h-screen bg-background text-foreground p-4">
          ...
          {messages.length === 0 ? (
            ...
            <>
              {messages.map((msg, index) => (
                <div key={msg.id || index} className="mb-3 p-2 rounded bg-secondary/30">
              ...
              {typingUsers.map((typingUser) => (
                <div
                  ...
                  className="mb-3 p-2 rounded bg-secondary/10 italic"
                >
        ...
        <div className="text-sm text-muted-foreground">
          {user
            ? `Connected as: ${user.displayName || "Anonymous User"}`
            : "Connecting..."}
        </div>
      </div>
    </div>
