...
Active Chat is a TanStack Start application deployed to Cloudflare Workers. Each room is a Cloudflare Durable Object that coordinates its active WebSocket participants. It broadcasts messages and typing state only; it does not write messages, participants, or room metadata to storage.
...
`wrangler.jsonc` declares the `CHAT_ROOMS` Durable Object binding and the `chat-rooms-v1` SQLite migration. SQLite is enabled because Durable Objects require a storage backend, but this app intentionally makes no storage calls.
...
Messages are ephemeral but not end-to-end encrypted. Cloudflare processes the WebSocket traffic required to relay it. Use a room-specific secret and avoid sending sensitive information that requires cryptographic end-to-end confidentiality.
